...
zpos -= zDelta;
pedalAngle = degrees(angleSum(radians(pedalAngle), speed/RADIUS_WHEEL)); //Pedal rotates as bike moves but this effect has been neutralised in the program // we'll be using sin(steering) and cos(steering) more than once
...
glPopMatrix();
glPushMatrix();
glTranslatef(0.0f,0.0f,WHEEL_OFFSET); XCylinder(ROD_RADIUS,WHEEL_LEN);
glPopMatrix();
/*****************************
...
* at once immediately.
* For each unit of time, the * handle slowly begins to align
...
bitmap_output(-1.25,-0.2,0.50,"8. MOVE LEFT - USE 'LEFT ARROW'"); bitmap_output(-1.25,-0.4,0.50,"9. MOVE RIGHT - USE 'RIGHT ARROW'");
bitmap_output(-1.25,-0.6,0.50,"10. USE MOUSE TO CHANGE THE ANGLE OF VIEWING");
    bitmap_output(-1.25,-0.8,0.50,"11. CONTINUE PRESSING + TO INCREASE THE SPEED");
